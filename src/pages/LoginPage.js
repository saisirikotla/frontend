...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!err.response) msg = 'Cannot connect to server. Please try again later.';
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'var(--gradient-hero)' }}>
      <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        <div style={{ marginTop: 14, padding: '9px 14px', background: 'rgba(79,142,247,0.07)', border: '1px solid rgba(79,142,127,0.18)', borderRadius: 'var(--radius-sm)', fontSize: 11, color: 'var(--text-secondary)', textAlign: 'center' }}>
          💡 Tip: Ensure you are connected to the Anurag University network for best performance.
        </div>
      </div>
    </div>
  );
